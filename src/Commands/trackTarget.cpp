...
*/
	...
	if(x == -1){
		consecutiveBadData++;
		std::cout << "trackTarget: Received bad data, retrying..." << std::endl;
		if(consecutiveBadData > 5) {
			//TODO Abort Shooting, fire in low goal only.
		}
		return;
	} else {
	...
	if (x < 159) {
		Robot::pixyController->SetServoPanAngle(Robot::pixyController->GetServoPanAngle() - delta);
	} else if (x > 161) {
		Robot::pixyController->SetServoPanAngle(Robot::pixyController->GetServoPanAngle() + delta);
	}
	} else {
